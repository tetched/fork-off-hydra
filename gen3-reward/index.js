...
async function main() {
  ...
  console.log('sudo account', encodeAddress(from.addressRaw, 63));
  const TREASURY = encodeAddress(TREASURY_ACCOUNT, 63);
  console.log('treasury account:', TREASURY);

  const transfers = [
    ...validators.map(address => api.tx.balances.forceTransfer(TREASURY, address, hdxToBN(6000))),
    ...nominators.map(address => api.tx.balances.forceTransfer(TREASURY, address, hdxToBN(1000)))
  ]
  ...
  await sudo.signAndSend(from, ({ events = [], status }) => {
    if (status.isInBlock) {
      ...
      console.log('transfers executed:', events.filter(({ event: { method }}) => method === 'Transfer').length);
    } else {
    ...
    events
      .filter(({ event: { section }}) => ['system', 'utility', 'sudo'].includes(section))
      .forEach(({ event: { data, method, section } }) => console.log(`event: ${section}.${method} ${data.toString()}`));
  });
}
